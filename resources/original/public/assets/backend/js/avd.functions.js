...
function ajaxFormError(xhr)
{
    if (xhr.status == 422) {    
        ...
        msgNotifica(false, '<ol>'+msgs+'</ol>', true, false);
...
function msgNotifica(retorno,msg,bt,gs)
{
	if(retorno === true){
		notify('Sucesso!',  msg, {
			hPos: 'right',
			icon: avdUrl+"/assets/backend/img/icons/notifique-ok.png",
			iconOutside: false,
			closeButton: bt,
		...
		notify('Erro na Aplicação!', msg, {
			hPos: 'center',
			icon: avdUrl+"/assets/backend/img/icons/notifique-erro-min.png",
			iconOutside: false,
			closeButton: bt,
			showCloseOnHover: false,
			groupSimilar: gs
