...
<div class="columns">
@if(isset($data))
	<form id="form-image"  method="post" action="{{route($upload['type'].'-category.update', ['$id' => $id, 'file' => $data->id] )}}" class="columns" enctype="multipart/form-data">
		<input name="_method" type="hidden" value="PUT">
		<input type="hidden" id="ac" name="ac" value="update">
@else
	<form id="form-image"  method="post" action="{{route($upload['type'].'-category.store', $id)}}" class="columns" enctype="multipart/form-data">
