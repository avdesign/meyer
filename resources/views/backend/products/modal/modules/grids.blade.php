@if(isset($data))
    <fieldset class="fieldset">
        ...
        <div id="update-grids">            
            @if($kit == 1)       
                @include('backend.products.modal.forms.grids-update-kits')
            @else
                @include('backend.products.modal.forms.grids-update')
            @endif
        </div>
    </fieldset>
@else
...
<script type="text/javascript">
    ...
    $( ".grid_colors" ).change(function() {
        ...
        $.get( base+"/painel/produto/{{$idpro}}/grids/"+opc+"/"+mod+"/"+stock+"/"+kit, function( data ) {
            $("#loader-grids").hide();
