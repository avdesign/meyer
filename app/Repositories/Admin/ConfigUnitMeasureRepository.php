...
use AVDPainel\Models\Admin\ConfigUnitMeasure as Model;
use AVDPainel\Interfaces\Admin\ConfigUnitMeasureInterface;

use Illuminate\Support\Facades\Gate;
use Illuminate\Foundation\Validation\ValidatesRequests;

class ConfigUnitMeasureRepository implements ConfigUnitMeasureInterface
{
    ...
    public function getAll($request)
    {
        ...
        if (empty($request->input('search.value'))) {            
            ...
            $query =  $this->model->where('unit','LIKE',"%{$search}%")
            ...
            $totalFiltered = $this->model->where('unit','LIKE',"%{$search}%")
                            ->where('name','LIKE',"%{$search}%")
                             ->count();
        }
    ...
    public function update($input, $id)
    {
        ...
        if ($update) {
            generateAccessesTxt(
                date('H:i:s').utf8_decode(
                ' Alterou a unidade de medida:'.$unit.
                ', Nome:'.$name.
                ', Ordem:'.$order.
                ', Status:'.$status.
                ' - Para:'.$input['unit'].
