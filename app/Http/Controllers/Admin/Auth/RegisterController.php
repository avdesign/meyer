...
class RegisterController extends Controller
{
    ...
    protected function index()
    {
        ...
        return view("{$this->view}.index", compact('data', 'title', 'confUser'));    
    }
   ...
   /**
    ...
    protected function create()
    {
        ...
        return view("{$this->view}.form", compact('options'));
    }
    ...
    protected function store(ReqModel $request)
    {
        ...
        if ($data) {            
            ...
            if ($data->status == 'Ativo') { 
                ...
                if ( $this->confUser->create($config) ) {
                    $i=0;
    ...
    protected function show($id)
    {
        ...
        return view("{$this->view}.myprofile", compact('data', 'id'));
    ...
    protected function update(ReqModel $request, $id)
    {
        ...
        $dataForm = $request->all();
        $profile  = $this->profile->setId($dataForm['profile_id']);
        $count    = count($this->permissions->getFilde('admin_id', $id));
        if( $this->interModel->update($dataForm, $id) ) {
            if ($dataForm['status'] == 'Ativo') {
                if ( !$this->permissions->deleteUser($id) && $count > 0) {
                    DB::rollBack();
                    $success = false;
    ...
    public function destroy($id)
    {
        ...
        if ( $this->interModel->delete($id) ) {
            ...
            if ( $this->permissions->deleteUser($id) && $count > 0) {
                ...
                DB::rollBack();
                $success = false;
                $message = 'Não foi possível excluir as permissões.';
            }
    ...
    */
    protected function excluded()
    {
        ...
        return view("{$this->view}.excluded", compact('title', 'confUser'));
    }
    ...
    protected function reactivateExcluded(Request $request)
    {
        ...
        if ($request['action'] == 'reactivate') {
            ...
            if ( $this->interModel->reactivate($id) ){
                ...
                $profile = $this->profile->getFilde('name', $user->profile);
                $i=0;
                foreach ($profile->permissions as $val) {
                        $insert = [
                            "module_id" => $val->module_id,
